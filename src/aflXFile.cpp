	...
	for(i=0;xfileData->GetChild(i,&xfileObject) == DXFILE_OK;i++)
	{
		...
		if(xfileObject->GetType(&xfileGUID) == DXFILE_OK)
		{
			if(xfileGUID == TID_D3DRMFrameTransformMatrix)
			{
				...
				xfileObject->Lock(&dwSize, (LPCVOID*)&frameData->matrix);
				xfileObject->Unlock();
			}
	...
	for(i=0;xfileData->GetChild(i,&xfileObject) == DXFILE_OK;i++)
	{
		...
		if(xfileObject->GetType(&xfileGUID) == DXFILE_OK)
		{
			if(xfileGUID == TID_D3DRMMaterial)
			{
				...
				//{
				//	pfMaterial[0],pfMaterial[1],pfMaterial[2],pfMaterial[3],
				//	//0.1f,0.1f,0.1f,1.0f,
				//	pfMaterial[0]*0.3f,pfMaterial[1]*0.3f,pfMaterial[2]*0.3f,1.0f,
				//	pfMaterial[5],pfMaterial[6],pfMaterial[7],0.0f,
				//	pfMaterial[8],pfMaterial[9],pfMaterial[10],0.0f,
				//	pfMaterial[4]
					...
					if(xfileAnimation->GetType(&xfileGUID) == DXFILE_OK)
					{
						...
						if(xfileGUID == TID_D3DRMAnimationKey)
						{
							...
							for(i=0;i<(INT)dwAnimation;i++)
							{
								...
								{
									//スケール
									animationKey.scale[dwTime] = *(NVector3*)pData;
								}
								else if(dwType == 2)
								{
									//ポジション
									animationKey.position[dwTime] = *(NVector3*)pData;
...
bool XFile::convertD3DX(MeshData* meshData,bool flag3DX)
{
	...
	if(!m_setTextureFullPath)
	{
		...
		{
			INT c = (INT)meshData->materialData[i].name.front().rfind('\\');
			if(c != -1)
			{
				meshData->materialData[i].name.push_back(meshData->materialData[i].name.front().substr(c + 1).c_str());
			}
		}
	}
	if(!flag3DX)
...
bool XFile::save(LPCWSTR fileName,FileObject* worldObject,DXFILEFORMAT format,bool flag3DX)
{
	...
	for(auto itAnime=fileObject->anime.begin();itAnime!=fileObject->anime.end();++itAnime)
	{
...
ID3DXFileSaveData* XFile::writeAnimation(ID3DXFileSaveObject* xfileSave,LPCWSTR name,ANIMATIONSET* anime)
{
	return writeAnimation(xfileSave,SJIS(name),anime);
}
ID3DXFileSaveData* XFile::writeAnimation(ID3DXFileSaveObject* xfileSave,LPCSTR name,ANIMATIONSET* anime)
{
	...
	if(hr != DXFILE_OK)
		return NULL;
	//std::map<WString,AnimationKey>::iterator itKey;
	for(auto itKey=anime->keys.begin();itKey!=anime->keys.end();++itKey)
	{
		//アニメーショングループの登録
		ID3DXFileSaveData* xfileAnime;
		xfileAnimeSet->AddDataObject(TID_D3DRMAnimation,NULL,NULL,0,NULL,&xfileAnime);
		xfileAnime->AddDataReference(SJIS(itKey->first.c_str()),NULL);

		//行列キー
		if(itKey->second.matrix.size())
		{
			INT objectSize = (sizeof(DWORD)*2 + sizeof(D3DMATRIX))*(INT)itKey->second.matrix.size();
	...
	*(LPDWORD)data = (DWORD)meshData->vertexIndex.size();
	...
	if(meshData->materialIndex.size())
	{
		...
		*(LPDWORD)data = (DWORD)meshData->materialIndex.size();
		...
		for(itMaterial=meshData->materialData.begin();itMaterial!=meshData->materialData.end();++itMaterial)
		{
			...
			//テクスチャネーム
			if((*itMaterial).name.front() != "")
			{
				LPCSTR data = (*itMaterial).name.front().c_str();
				//\を\\に変換
				INT i,j;
				INT sCount = 0;
				for(i=0;data[i];i++)
				...
				convertString.reserve(((*itMaterial).name.front().length()+sCount));
				...
				(*itMaterial).name.front() = convertString.c_str();
				String outName = SJIS(convertString);
				ID3DXFileSaveData* xfileData;
				hr = xfileMaterial->AddDataObject(TID_D3DRMTextureFilename,NULL,NULL,
					(DWORD)outName.length()+1,(LPVOID)outName.c_str(),&xfileData);
				xfileData->Release();
			}
	...
	if(meshData->boneData.size())
	{
		...
		for(itBone=meshData->boneData.begin();itBone!=meshData->boneData.end();++itBone)
		{
			...
			String name = SJIS((*itBone).name.c_str());
			INT boneNameSize = (INT)name.length()+1;
			INT boneIndexSize = (INT)itBone->weight.size() * sizeof(INT);
			INT boneWeightSize = (INT)itBone->weight.size() * sizeof(FLOAT);
			INT dataSize = boneNameSize+boneIndexSize+boneWeightSize+sizeof(DWORD)+sizeof(D3DMATRIX);
			LPBYTE dataObject = NEW BYTE[dataSize];
			LPBYTE data = dataObject;
			//対応フレームネーム

			CopyMemory(data,name,boneNameSize);
			data += boneNameSize;
			//ボーンリストサイズ
...
ID3DXFileSaveData* XFile::writeFrame(ID3DXFileSaveObject* xfileSave,ID3DXFileSaveData* xfileSaveData,FrameData* frameData)
{
	...
	if(xfileSaveData)
	{
		hr = xfileSaveData->AddDataObject(TID_D3DRMFrame,SJIS(frameData->name.c_str()),NULL,0,NULL,&xfileFrame);
		if(hr != DXFILE_OK)	//失敗
			return NULL;
	}
	else
	{
		hr = xfileSave->AddDataObject(TID_D3DRMFrame,SJIS(frameData->name.c_str()),NULL,0,NULL,&xfileFrame);
		if(hr != DXFILE_OK)	//失敗
			return NULL;
	}
	//ローカル行列
	...
	hr = xfileFrame->AddDataObject(TID_D3DRMFrameTransformMatrix,
		NULL,NULL,sizeof(D3DMATRIX),&frameData->matrix,&xfileMatrix);
	xfileMatrix->Release();
